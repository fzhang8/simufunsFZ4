...
LD_plot <- function(D,FstSingulars,Demeanlevel,title = "test plot"){
	...
	p <- ggplot(dtset, aes(x=D, y=FstSingulars, group=Demeanlevel)) + 
				...
				labs(title = paste("Compare effects of LD coefficient on first singular value for marginal effect of observed G matrix\n between demean or not by '",
														ifelse(title == "obsmean","observed mean",ifelse(title == "latmean","latent mean","expectation")),"'",sep = "")) +
